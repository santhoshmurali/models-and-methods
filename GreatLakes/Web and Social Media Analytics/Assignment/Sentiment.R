...
TweetData =  searchTwitteR(searchString = "GST",n = 2000, resultType = 'mixed')
...
enStopWords<- c((stopwords('en')),c("rt", "use", "used", "via", "amp", "gst", "http","https"))
...
WordCorr <- apply_as_df(TweetDataCorpus[1:935], word_cor, word = "people", r=.25) #Change
plot(WordCorr)
...
head(unique(df[grep("people", df$text), ]), n=10)


##### Find association with a specific keyword in the tweets - 
findAssocs(TweetTDM, "people", 0.2)
findAssocs(TweetTDM, "price", 0.2)
